...
include('../functions/common_function.php');
    ...
    if($name=='' or $email=='' or $address=='' or $city=='' or $state=='' or $zipcode=='' ){
        echo "<script>alert('Please fill all the available fields')</script>";
        exit();
    }else{
        //insert query
        $insert_delivery_details="insert into `delivery_table` (name,email,address,city,state,zipcode) values('$name', '$email', '$address', '$city', '$state', '$zipcode')";
        $result_query=mysqli_query($con,$insert_delivery_details);
        if($result_query){
            echo"<script>Product Ordered Successfully</script>";
        }
    }
